...
for run_case in flags:
    ...
    file_name = 'Frm{}_S-col1-{}-col2-{}_M-bm1-{}-bm2-{}'.format(run_case,
                                                                 frm1_col_stiff,
                                                                 frm2_col_stiff,
                                                                 frm1_bm_weight,
                                                                 frm2_bm_weight)
    ...
    mass_frm1 = frm1_bm_weight / (sap2000.GRAVITY * 12)
    ...
    mass_frm2 = frm2_bm_weight / (sap2000.GRAVITY * 12)
    ...
    usr_period = 2 * pi * 1 / sqrt(
            1 / 2 * (
                    (stiff_frm1 + ke[0]) / mass_frm1 +
                    (ke[0] + stiff_frm2) / mass_frm2
            ) -
            sqrt(
                    ((stiff_frm1 + ke[0]) / mass_frm1 -
                     (ke[0] + stiff_frm2) / mass_frm2) ** 2
                    + 4 * ke[0] ** 2 / mass_frm1 / mass_frm2
            )
    )

    sap_period_frm1 = min(res.period)
    sap_period_frm2 = min(res.period)
